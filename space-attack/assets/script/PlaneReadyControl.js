...
cc.Class({
    ...
    initPlaneList(){
        this.planeList = [];
        let rotateForPlanes = [0, 0, 0, 0, 0, 0];
        for (let i=0; i<this.allPlanes.length; i++){
            let obj = {
                id: i,
                name: Global.allPlaneNames[i],
                isSelected: i == Global.selectedPlaneNum,
                plane: this.allPlanes[i],
                rotate: rotateForPlanes[i],
                itemClickCallback: (index)=>{
                    this.selectHeroPlane(index)
                },
            }
            let item = cc.instantiate(this.planePrefab)
            item.getComponent('PlaneSelectItem').initItem(obj)
            this.scrollViewContent.addChild(item)
            this.planeList.push(item)
        }
    },

    selectHeroPlane(index){
        cc.audioEngine.playEffect(this.buttonAudio, false);
        Global.selectedPlaneNum = index;
        for(let i=0; i<this.planeList.length; i++){
            this.planeList[i].getComponent('PlaneSelectItem').updateItem(i==index)
        }
    },
